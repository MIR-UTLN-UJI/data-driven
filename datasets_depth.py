...
class RelocDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx > seq_limit-ns-1:
            idx= idx-ns
        tensor1=self.data[idx]['image']
        tensor2=self.data[idx+1]['image']
        for i in range(ns-1):
    ...
    def __init__(self, path):
        ...
        for data, df, global_depths in [(train_data, train_df, train_global_depths),
            ...
            for line in tqdm.tqdm(df.values):
                ...
                #print("///////////////////////////")
                #image[3,:,:]=image[0,:,:]
                #image= torch.cat((image, image[0:1,:,:]), 0)

                depth_file=image_file.split('.')[0]+'.npy'
                #depth_file[-3:len(depth_file)]='npy'
                #print(depth_file)
                #print("///////////////////////////")
                depth_map=torch.tensor(np.load(os.path.join(path, depth_file)))
                ...
                #image= torch.cat((image, image[0:1,:,:]), 0)
                #print(image.shape)
                image= torch.cat((image, depth_map), 0) #concat image with depth image 4,256,455
                #print(image.shape)
                #print("//////////////////////////////:")
