...
class RelocDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ns=2
        idx=idx-1
        limits={'seq1':36,'seq2':231,'seq3':67}
        image_name=self.data[idx]['image_file']
        seq=image_name.split('.')[0].split('/')[0]
        seq_limit=limits[seq]
        if idx > seq_limit-ns:
            idx= idx-ns+1
        #print(idx)
        tensor1=self.data[idx]['image']
        last_image_idx=idx+1
        tensor2=self.data[last_image_idx]['image']
        for i in range(ns-1):
            tensor1=torch.cat((tensor1,tensor2),0)
            #tensor2=self.data[idx+i+2]['image']
        new_image=tensor1
        return {
            'image_file': self.data[last_image_idx]['image_file'],
            'image': new_image, #self.data[idx]['image'],
            'w_t_c': self.data[last_image_idx]['w_t_c'],
            'c_q_w': self.data[last_image_idx]['c_q_w'],
            'c_R_w': self.data[last_image_idx]['c_R_w'],
            'K': self.data[last_image_idx]['K'],
            'w_P': self.data[last_image_idx]['w_P'],
            'c_p': self.data[last_image_idx]['c_p'],
            'xmin': self.data[last_image_idx]['xmin'],
            'xmax': self.data[last_image_idx]['xmax']
        }
